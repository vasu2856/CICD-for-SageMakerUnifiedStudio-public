...
> **[דומיינים IAM + IdC]** ה-CLI הזה תומך בדומיינים של SMUS המשתמשים באימות מבוסס IAM ו-IAM Identity Center (IdC). עבור דומיינים מבוססי IdC, ייתכן שתידרש הגדרה נוספת (רשת VPC, הרשאות Lake Formation, מדיניות IAM מוטבעת) — ראו את סקריפטי ההגדרה בכל תיקיית דוגמה.
